...
| Integrations | IN PROGRESS (this session) | The Integration Hub now tracks 35 catalog entries with real adapter/config/test foundations. Governed depth includes Slack, Telegram, WhatsApp, Instagram, Snapchat, Discord, Teams, Gmail, Google Calendar/Drive, GitHub/GitLab, Notion, Jira, Dropbox, OneDrive, Stripe, Shopify, WooCommerce, Amazon, HubSpot, Salesforce, and the platform connectors. Capability depth still varies by provider (read-only, approval-gated writes, or webhook foundations), and external accounts remain credential/live-validation dependent. |
...
- Render now implements fixed `cloud.service.read` and approval-gated
