...
class MakeWebhookAdapter(IntegrationAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        self.auth_header = auth_header or settings.connector_credential(
            "MAKE_WEBHOOK_AUTH_HEADER", settings.make_webhook_auth_header
        )
        self.auth_value = auth_value or settings.connector_credential(
            "MAKE_WEBHOOK_AUTH_VALUE", settings.make_webhook_auth_value
        )
        self._client = client
    ...
    async def run_capability(self, capability_id: str, arguments: dict) -> object:
        ...
        if not result.success:
            raise RuntimeError(result.error or "Make scenario trigger failed")
        return {
            "provider": IntegrationProvider.MAKE.value,
