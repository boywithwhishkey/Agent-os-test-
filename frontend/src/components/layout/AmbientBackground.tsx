  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden" aria-hidden>
      ...
      <motion.div className="absolute inset-0" style={{ y: meshY }}>
        <div className="absolute left-[-10%] top-[-15%] h-[55vh] w-[55vh] rounded-full bg-accent-violet/[0.07] blur-[120px] animate-drift" />
        <div className="absolute bottom-[-15%] right-[-10%] h-[60vh] w-[60vh] rounded-full bg-accent-blue/[0.06] blur-[130px] animate-drift-slow" />
      </motion.div>
      ...
      {!reduceMotion && (
        <div className="absolute inset-0">
          {points.map((p, i) => (
            <span
              key={i}
              className="absolute h-1 w-1 rounded-full bg-accent-violet-soft/40 animate-float"
