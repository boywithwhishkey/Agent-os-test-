...
// Not a component, so it cannot call useT() — the caller passes `t` in.
function describeError(
  ...
  if (error instanceof ApiError) {
    if (error.isTimeout) {
      return { title: t("states.timedOut"), description: error.detail, icon: <Clock className="h-5 w-5" /> };
    }
    if (error.isNetworkError) {
      return {
        title: t("states.cantReachApi"),
        // The detail here is generated by our own client, not the provider, so
        // it is product copy and gets translated rather than echoed in English.
        description: t("states.networkHint"),
        icon: <WifiOff className="h-5 w-5" />,
      };
    }
    if (error.status === 404) {
      return { title: t("states.notFound"), description: error.detail, icon: <SearchX className="h-5 w-5" /> };
    }
    return {
  ...
  return {
    ...
    description: error instanceof Error ? error.message : t("states.unexpectedError"),
    icon: <AlertTriangle className="h-5 w-5" />,
  };
}
  ...
  return (
    <div className="glass-soft flex flex-wrap items-center justify-between gap-3 rounded-xl border border-hairline px-4 py-3.5 text-sm">
      <span className="flex items-center gap-2.5">
        ...
        <span>
          <span className="block font-medium text-content-primary">{t("states.authRequiredTitle")}</span>
          <span className="block text-xs text-content-muted">
            {t("states.authRequiredBody", { detail: description })}
          </span>
        </span>
      </span>
      <Link
    ...
    <div className="glass-soft flex flex-wrap items-center gap-3 rounded-xl border border-accent-amber/25 px-4 py-3.5 text-sm">
      ...
      <span className="min-w-0">
        <span className="block font-medium text-content-primary">
          {t("states.serviceUnavailableTitle")}
        </span>
...
function ForbiddenState({ description }: { description: string }) {
  ...
  return (
    <div className="glass-soft flex flex-wrap items-center gap-3 rounded-xl border border-accent-red/25 px-4 py-3.5 text-sm">
      ...
      <span className="min-w-0">
        <span className="block font-medium text-content-primary">{t("states.permissionDeniedTitle")}</span>
        <span className="block text-xs text-content-muted">{description}</span>
      </span>
    </div>
  );
...
export function ErrorState({ error, onRetry }: { error: unknown; onRetry?: () => void }) {
  const t = useT();
  // Known machine codes get localised copy; anything else keeps the server's
  // own words rather than being machine-translated.
  const described = (e: unknown) =>
    e instanceof ApiError && e.code
      ? t(`errorCodes.${e.code}` as Parameters<typeof t>[0]) || e.detail
  ...
  if (error instanceof ApiError && error.isUnauthorized) {
    return <AuthRequiredState description={described(error)} />;
  }
  if (error instanceof ApiError && error.isForbidden) {
    return <ForbiddenState description={described(error)} />;
  }
  if (error instanceof ApiError && error.isUnavailable) {
    return <ServiceUnconfiguredState description={described(error)} />;
  }

  const { title, description, icon } = describeError(error, t);
